...
// 使用 DeepSeek 官方 logo 路径（deepseek.com 品牌标识）
...
const sharp = require("C:/Users/fengq/AppData/Local/npm-cache/_npx/1e7f6d9597241db0/node_modules/sharp");

const root = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const assets = path.join(root, "assets");

// DeepSeek 官方鲸鱼标识路径（viewBox 0 0 24 24，来源：官方品牌 SVG）
...
// 应用图标：渐变蓝圆角底 + 白色官方标识（标识约占画布 62%，居中）
...
<rect x="0" y="0" width="256" height="256" rx="58" fill="url(#g)"/>
<g transform="translate(51,52) scale(6.4)" fill="#ffffff" fill-rule="evenodd">
  <path d="${OFFICIAL_PATH}"/>
</g>
</svg>`;

// 导出官方标识（透明底，蓝色），供文档/标题栏使用
const MARK_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
<path d="${OFFICIAL_PATH}" fill="#4D6BFE" fill-rule="evenodd"/>
</svg>`;

async function main() {
  fs.mkdirSync(assets, { recursive: true });
  fs.writeFileSync(path.join(assets, "logo-mark.svg"), MARK_SVG);

  // 优先使用 assets/icon.svg（用户可自行修改），否则用内置 SVG 兜底
  const iconSvgPath = path.join(assets, "icon.svg");
  let iconSvg;
  if (fs.existsSync(iconSvgPath)) {
    iconSvg = fs.readFileSync(iconSvgPath, "utf8");
    console.log("using assets/icon.svg (user-provided)");
  } else {
    iconSvg = ICON_SVG;
    fs.writeFileSync(iconSvgPath, ICON_SVG);
    console.log("using built-in icon.svg (written to assets/)");
  }

  // 渲染各尺寸 PNG（256 用于 ICO）
  ...
  fs.writeFileSync(path.join(assets, "icon.png"), pngs.get(256));

  // 生成 ICO（Vista+ 支持 PNG 压缩条目，所有尺寸均可）
  const entries = [16, 24, 32, 48, 64, 128, 256];
  ...
  for (const size of entries) chunks.push(pngs.get(size));
  fs.writeFileSync(path.join(assets, "icon.ico"), Buffer.concat(chunks));
